...
const activeUnitIds = new Set([FUNDACION_DANTE_UNIT_ID, AXEN_ENERGY_UNIT_ID, AXEN_HEALTH_UNIT_ID]);
const scopedSeedUnits = seedUnits.filter((unit) => activeUnitIds.has(unit.id));
const scopedSeedPositions = seedPositions.filter((position) => activeUnitIds.has(position.businessUnitId));
const scopedPhysicalChecklistFormats = (physicalChecklistFormats as PhysicalChecklistFormat[]).filter((format) => activeUnitIds.has(format.businessUnitId));
...
function mergePositionSeedMetadata(saved: Position[]) {
  const activeSaved = saved.filter((position) =>
    activeUnitIds.has(position.businessUnitId)
  ...
  return mergedSeeds.map((position) => {
...
function mergeUnitSeeds(saved: BusinessUnit[]) {
  const scopedSaved = saved.filter((unit) => activeUnitIds.has(unit.id));
  const savedById = new Map(scopedSaved.map((unit) => [unit.id, unit]));
  return scopedSeedUnits.map((unit) => ({ ...unit, ...(savedById.get(unit.id) ?? {}) }));
}
...
export function useCatalog() {
  ...
  function addPosition(position: Position) {
    if (!activeUnitIds.has(position.businessUnitId)) return;
